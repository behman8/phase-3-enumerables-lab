...
def print_spiciest_foods(spicy_foods)
  filtered_food = spicy_foods.filter do |food|
    if food[:heat_level] > 5
      food
    end
  end

  output_food = filtered_food.map do |food|
    peppers = "🌶" * food[:heat_level]
    puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{peppers}"
  end
end
...
def average_heat_level(spicy_foods)
  food_spicy_level = spicy_foods.map do |food|
    food[:heat_level]
  end

  numerator = food_spicy_level.reduce(0) { |a,v| a + v }
  denominator = food_spicy_level.count
